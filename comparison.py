...
def compare_results(dataset):
    ...
    for mode in modes:
        ...
        # Path to summary stats (single row)
        summary_path = f"results/{folder_name}/results_summary.csv"
        ...
        if os.path.exists(asset_path):
            ...
            print(f"Warning: Results for mode '{mode}' not found at {asset_path}")
    ...
    if summary_dfs:
        ...
        cols = ['Dataset', 'Reward_Mode', 'Initial_Asset', 'Final_Asset', 'Sharpe_Ratio', 'MDD', 'Turnover_Rate']
        ...
        print("-" * 95)
        print(f"{'Mode':<10} | {'Init Asset':<12} | {'Final Asset':<12} | {'Sharpe':<8} | {'MDD':<8} | {'Turnover':<8}")
        print("-" * 95)
        
        for _, row in final_summary.iterrows():
            print(f"{row['Reward_Mode']:<10} | {row['Initial_Asset']:<12} | {row['Final_Asset']:<12} | {row['Sharpe_Ratio']:<8} | {row['MDD']:<8} | {row['Turnover_Rate']:<8}")
        print("="*95 + "\n")
    else:
